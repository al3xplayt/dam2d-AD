...
public class TransaccionesPrincipal {
	...
	public static void main(String[] args) {
		try {
			int opcion = 0;
			int codigoEscritor, codigoLibro, agno_publicacion, cant, codigoTienda;
			int numero_paginas;
			...
			do {
					...
					for (int i = 0; i < cantidad; i++) {
						...
						if (!vivo) {
							fechaFallecimiento = Teclado
									.leerCadena("Introduzca la fecha de fallecimiento del escritor (si aplica): ");
						} else {
				...
				case 2:
					...
					break;
				case 3:
					// Eliminar una tienda y sus disponibilidades de la base de datos.
					codigoTienda = Teclado.leerEntero("Introduzca el código de la tienda a eliminar: ");
					if (AccesoTienda.consultarTienda(codigoTienda) == null) {
						System.out.println("La tienda no existe.");
						break;
					} else {
						filasEliminadas = AccesoTienda.eliminarTiendaDisponibilidades(codigoTienda);
						System.out.println("Se han eliminado " + filasEliminadas[0] + " disponibilidades y " + filasEliminadas[1]
								+ " tiendas de la base de datos.");
					}
					break;
