...
public class PrincipalZona {
	public static void escribirMenu() {
		...
		System.out.println("2) Consultar una zona por c�digo.");
	...
	//Para actualizar un nodo update value0
			...
			try {
				...
				case 1:
					...
					if (listaZonas.isEmpty()) {
						...
						System.out.println("Se ha encontrado " + listaZonas.size() + " zonas.");
					}
				...
				case 4:
					director = Teclado.leerCadena("Introduce el director de la zona: ");
					if((listaZonas = AccesoZona.consultarPorDirector(director)).isEmpty()) {
                        System.out.println("No hay zonas en la base de datos.");
					} else {
						AccesoZona.eliminarPorDirector(director);
						System.out.println("Se han eliminado "+ listaZonas.size() +" zonas.");
					}
